...
def calculate_similarities(query_features, all_features):
    ...
    for i in range(sim.shape[0]):
        for j in range(sim.shape[1]):
            if i == 0 and j == 0:
                ...
                sim[i][j] = max(sim[i][j-1], sim[i][j])
            elif j == 0:
                sim[i][j] = max(sim[i-1][j], sim[i][j])
            else:
                sim[i][j] = max(sim[i-1][j-1]+sim[i][j],sim[i-1][j],sim[i][j-1])
    ...
    return sim[sim.shape[0]-1][sim.shape[1]-1]
    ...
    if results is None:
        ...
        for _,id in enumerate(query_indexs):
            ...
            for __,temp_feature in enumerate(global_feattures):
                now_similarities = p.apply_async(calculate_similarities, args=(query_features, temp_feature))
